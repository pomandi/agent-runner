...
async def push_landing_page_to_git(config_path: str, slug: str) -> Dict[str, Any]:
    """
    Push landing page config to git repository using GitHub API.
    ...
    activity.logger.info(f"Pushing landing page to git via GitHub API: {slug}")

    # GitHub settings
    GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")
    REPO_OWNER = "pomandi"
    REPO_NAME = "pomandi-landing-pages"
    BRANCH = "main"
    ...
    try:
        ...
        # File path in repo
        file_path = f"src/config/pages/{slug}.json"
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if check_response.status_code == 200:
                ...
                activity.logger.info(f"File exists, will update (SHA: {existing_sha[:7]})")
            ...
            if response.status_code in [200, 201]:
                ...
                commit_sha = result_data.get("commit", {}).get("sha", "unknown")

                activity.logger.info(f"Successfully pushed landing page: {slug} (commit: {commit_sha[:7]})")
